...
class BertModelForPrompt(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    def forward(
        ...
        # embedding_output = embedding_output.view(batch_size, num_clicked, -1, 768)
        '''
        for idx in range(batch_size):
            for idy in range(32):
                embedding_output[idx][user_place+idy] = hist_news[idx][idy]
        '''
        embedding_output[:, user_place:user_place+32] = hist_news
        embedding_output = embedding_output.unsqueeze(dim=1)
        embedding_output_repeat = embedding_output.repeat(1, num_clicked, 1, 1)
        extended_attention_mask = extended_attention_mask.repeat(1, num_clicked, 1, 1)
        '''
        for idx in range(batch_size):
            for idy in range(num_clicked):
                embedding_output_repeat[idx][idy][candidate_place] = curr_news[idx][idy]
        '''
        embedding_output_repeat[:, :num_clicked, candidate_place] = curr_news
        embedding_output_repeat = embedding_output_repeat.view(batch_size*num_clicked, -1, 768)
        extended_attention_mask = extended_attention_mask.view(batch_size*num_clicked, 1, 1, 50)
        encoder_outputs = self.encoder(
        ...
        return prediction_scores # (sequence_output, pooled_output) + encoder_outputs[1:],