...
class BaseEvaluator(ABC):
    def __init__(self, dataset: Dataset):
        ...
        #self.prob_predictions = torch.zeros(self.samples).cuda()
        self.begin = 0
...
class SlowEvaluator(BaseEvaluator):
    ...
    def eval_batch(self, logits: Tensor, impression_ids: Tensor):
        ...
        print(probs[:10])
        #step = probs.size(0)
        #self.prob_predictions[self.begin: self.begin+step] = probs
        #self.begin = self.begin + step
        print('0.5', time.time() - b)
        c = time.time()
        print(impression_ids.size())
        impression_ids = impression_ids.detach().cpu().tolist()
        print(impression_ids[:10])
        print('1', time.time() - c)
        d = time.time()
        self.prob_predictions.extend(probs)
        self.impression_ids.extend(impression_ids)
        print('2', time.time() - d)
