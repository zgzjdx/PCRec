...
class PolyAttention(nn.Module):
    ...
    def forward(self, embeddings, attn_mask, bias=None):
        ...
        # attn_mask[:,0] = True
        # weights = weights.masked_fill_(~attn_mask.unsqueeze(dim=1), -float('inf'))
        weights = weights.masked_fill_(~attn_mask.unsqueeze(dim=1), 1e-30)
        #batch_mask = torch.sum(attn_mask.int(), dim=1, keepdim=True) > 0
        #weights = weights.masked_fill_(~batch_mask.unsqueeze(-1), 0.0)
...
class SelfAdditiveAttention(nn.Module):
    def __init__(self, hidden_dim=768, head_num=12):
        super().__init__()
        self.head_num = head_num
        assert hidden_dim % head_num == 0, "hidden_dim {} must divide head_num {}".format(hidden_dim, head_num)
        self.head_dim = hidden_dim // head_num
    ...
    def forward(self, hist_pooled_outputs, curr_pooled_outputs, hist_mask, len):
        # Truncate the history to the recent `length` entries
        if len > 0 and len < hist_pooled_outputs.size(1):
            hist_pooled_outputs = hist_pooled_outputs[:, -len:, :]
            hist_mask = hist_mask[:, -len:]
        key = self.keyProject(hist_pooled_outputs)
        key = self.transpose_for_scores(key)
        masked_hist_pooled_outputs = hist_pooled_outputs.masked_fill(~hist_mask.unsqueeze(dim=-1), 1e-30)
        query = torch.mean(masked_hist_pooled_outputs, dim=1, keepdim=True)
        query = self.transpose_for_scores(self.queryProject(query))
        value = self.valueProject(hist_pooled_outputs)
        value = self.transpose_for_scores(value)
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query, key.transpose(-1, -2))
        attention_scores = (attention_scores / math.sqrt(self.head_dim))
        # Apply the mask to the attention scores
        hist_mask = hist_mask.unsqueeze(1).unsqueeze(2)  # [B, 1, 1, history_click_num]
        attention_scores = attention_scores.masked_fill(~hist_mask, -1e4)
        attention_probs = torch.softmax(attention_scores, -1)
        attn_output = torch.matmul(attention_probs, value)
        # [batch_size, signal_length, head_num, head_dim]
        attn_output = attn_output.permute(0, 2, 1, 3).contiguous()
        new_shape = attn_output.size()[:-2] + (self.head_dim * self.head_num,)
        attn_output = attn_output.view(*new_shape)
        score = torch.matmul(attn_output, curr_pooled_outputs.permute(0, 2, 1)).squeeze(dim=1)
        return score
